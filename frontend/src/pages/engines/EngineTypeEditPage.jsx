...
function EngineTypeEditPage() {
	...
	useEffect(() => {
		const fetchEngineType = async () => {
			try {
				const response = await api.get(`/api/vehicles/engines/${id}`);
				const engineType = response.data;
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			await api.put(`/api/vehicles/engines/${id}`, data);
			...
			<div className='flex justify-center items-start min-h-screen px-6 pt-12'>
				<div className='bg-[#101935] rounded-xl p-8 w-full max-w-[700px]'>
					<div className='mb-8 text-center'>
						<h1 className='text-2xl text-white font-medium'>Edytuj typ silnika</h1>
						<p className='text-sm text-white/60 mt-1'>Zaktualizuj dane typu silnika</p>
					</div>
...
export default EngineTypeEditPage;
