...
function VehicleAddPage() {
	...
	const [enginges, setEngines] = useState([]);
	...
	useEffect(() => {
		const fetchEngines = async () => {
			try {
				const response = api.get('/api/vehicles/engines');
				setEngines((await response).data);
			} catch (err) {
				console.log(err);
			}
		};
					...
					<form onSubmit={handleSubmit} className='space-y-4'>
						<div>
							...
							<select
								...
								className='w-full bg-[#0B122B] border border-white/10 rounded-lg px-3 py-2 text-white placeholder:text-white/30 focus:outline-none focus:border-indigo-500 transition appearance-none'>
						...
						<button
							className='flex items-center gap-2 bg-[#FDB52A] text-black px-5 py-2.5 rounded-lg hover:bg-[#e6a823] transition'
							type='submit'>
