...
function BrandAddPage() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			await api.post('/api/vehicles/brands', data);
			navigate('/brands');
		} catch (err) {
			console.log(err);
			...
			<div className='flex justify-center items-start min-h-screen px-6 pt-12'>
				<div className='bg-[#101935] rounded-xl p-8 w-full max-w-[700px]'>
					<div className='mb-8 text-center'>
						<h1 className='text-2xl text-white font-medium'>Dodaj markę</h1>
						<p className='text-sm text-white/60 mt-1'>Utwórz nową markę pojazdu</p>
					</div>
