...
function UnitOfMeasureEditPage() {
	...
	useEffect(() => {
		const fetchUnit = async () => {
			try {
				const response = await api.get(`/api/units-of-measure/${id}`);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			await api.put(`/api/units-of-measure/${id}`, data);
			...
			<div className='flex justify-center items-start min-h-screen px-6 pt-12'>
				<div className='bg-[#101935] rounded-xl p-8 w-full max-w-[700px]'>
					<div className='mb-8 text-center'>
						<h1 className='text-2xl text-white font-medium'>Edytuj jednostkę miary</h1>
						<p className='text-sm text-white/60 mt-1'>Zaktualizuj dane jednostki</p>
					</div>

					<form onSubmit={handleSubmit} className='space-y-4'>
						<div>
							<label className='block text-sm text-white/70 mb-1'>Nazwa jednostki *</label>
							<input
								name='name'
								value={formData.name}
								...
								type='text'
								required
								placeholder='np. szt, litr, kg, metr'
						...
						<div>
							...
							<textarea
								...
								onChange={handleInputChange}
								rows='3'
								placeholder='Dodatkowe informacje o jednostce...'
...
export default UnitOfMeasureEditPage;
